...
def calcular_momentum_e_stats(stats):
    """
    Calcula momentum_score baseado nas estatísticas do jogador
    
    Args:
        stats (dict): Estatísticas do jogador
        
    Returns:
        dict: Estatísticas com momentum_score incluído
    """
    try:
        # Componentes do momentum (peso de cada fator)
        momentum_score = 0
        
        # 1. Win 1st Serve (peso 30%)
        win_1st_serve = stats.get('win_first_serve', 0)
        momentum_score += (win_1st_serve * 0.3)
        
        # 2. Break Points Converted (peso 25%)
        bp_converted = stats.get('break_points_converted', 0)
        momentum_score += (bp_converted * 0.25)
        
        # 3. Total Points Won (peso 20%)
        total_points = stats.get('total_points_won', 0)
        if total_points > 0:
            # Normalizar para percentual (assumindo máximo 150 pontos)
            points_percentage = min(100, (total_points / 150) * 100)
            momentum_score += (points_percentage * 0.2)
        
        # 4. Winners vs Unforced Errors (peso 15%)
        winners = stats.get('winners', 0)
        errors = stats.get('unforced_errors', 0)
        if (winners + errors) > 0:
            winner_ratio = (winners / (winners + errors)) * 100
            momentum_score += (winner_ratio * 0.15)
        
        # 5. Aces (peso 10%)
        aces = stats.get('aces', 0)
        if aces > 0:
            # Normalizar aces (assumindo máximo 20 aces)
            aces_percentage = min(100, (aces / 20) * 100)
            momentum_score += (aces_percentage * 0.1)
        
        # Garantir que momentum_score está entre 0 e 100
        momentum_score = max(0, min(100, momentum_score))
        
        # Adicionar momentum_score às estatísticas
        stats_com_momentum = stats.copy()
        stats_com_momentum['momentum_score'] = round(momentum_score, 1)
        
        return stats_com_momentum
        
    except Exception as e:
        print(f"[ERROR] Erro ao calcular momentum: {e}")
        stats_com_momentum = stats.copy()
        stats_com_momentum['momentum_score'] = 0
        return stats_com_momentum
    ...
    try:
        ...
        if data.get('success') == 1 and 'results' in data:
            ...
            if results and len(results) > 0:
                ...
                return {
                    'stats_jogador1': calcular_momentum_e_stats(stats_j1),
                    'stats_jogador2': calcular_momentum_e_stats(stats_j2)
                }
